...
describe('Blog app', function() {
  ...
  it('user can like a blog', function () {
    cy.contains('New Blog').click()
    cy.contains('Create new blog')
    cy.get('#title').type('BLOG_1')
    cy.get('#author').type('Cypress automated system test')
    cy.get('#url').type('https://www.iwantoneofthose.com/')
    cy.get('#createBlogButton').click()
    cy.contains('BLOG_1')
    cy.get('#BLOG_1viewButton').click()
    cy.contains('likes: 0')
    cy.get('#BLOG_1likeButton').click()
    cy.contains('likes: 1')
    })
  ...
  it('user can remove a blog that they created', function () {
    ...
    cy.get('#BLOG_1likeButton').should('not.be.visible')
    })
  ...
  it('blogs are ordered according to likes', function () {
    ...
    cy.wait(2000)
    cy.contains('likes: 5')
    cy.get('.blog').eq(0).should('contain', 'BLOG_C')
    cy.get('.blog').eq(1).should('contain', 'BLOG_B')
    cy.get('.blog').eq(2).should('contain', 'BLOG_A')
    })
  })
})
})